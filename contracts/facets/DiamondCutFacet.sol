...
contract DiamondCutFacet is IDiamondCut {
    ...
    function diamondCut(
        ...
        diamondCutInternal(_diamondCut, _init, _calldata);
    }
    ...
    function addFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            ...
            ds.facetAddressAndSelectorPosition[selector] = Diamond.FacetAddressAndSelectorPosition(_facetAddress, selectorCount);
    ...
    function replaceFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            bytes4 selector = _functionSelectors[selectorIndex];
            require(selector != bytes4(0x1f931c1c) , "diamonCut function is immutable");
            address oldFacetAddress = ds.facetAddressAndSelectorPosition[selector].facetAddress;
    ...
    function removeFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
       // require(_functionSelectors != diamondCutSelector , "diamonCut function is immutable");
        ...
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            bytes4 selector = _functionSelectors[selectorIndex];
            require(selector != bytes4(0x1f931c1c) , "diamonCut function is immutable");
            Diamond.FacetAddressAndSelectorPosition memory oldFacetAddressAndSelectorPosition = ds.facetAddressAndSelectorPosition[selector];
